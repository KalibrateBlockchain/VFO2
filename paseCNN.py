...
def train_paseCNN_class_models(train_loader, test_loader, k_round):
    pCNN_model = get_model()

    if CUDA:
        pCNN_model = pCNN_model.cuda()

    optimizer = optim.Adam(filter(lambda p: p.requires_grad, pCNN_model.parameters()), lr=LR, weight_decay=WEIGHT_DECAY)
    if HAS_SCHEDULER:
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=PATIENCE, factor=FACTOR, eps=EPS)
    losses = []
    ...
    print('Done '+args.MODEL+' Training')
...
parser.add_argument('--MODEL', type=str, default='paseCNN_model')
parser.add_argument('--HAS_SCHEDULER', type=int, default=0)
...
if args.data == 'All':
    pos_covid_csv = 'positive_speakerid.csv'
        
    neg_covid_csv = 'negative_speakerid.csv'
