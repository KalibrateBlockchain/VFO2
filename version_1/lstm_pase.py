...
#         print(data.shape)
        ...
        covid_class = model(data.squeeze(0).transpose(1,2))
...
def test_epoch(model, dataloader, optimizer, mode):
    ...
    with torch.no_grad():
        ...
        for i, (data, labels, audio_type, spks) in enumerate(dataloader):
            ...
            logits = model(data.squeeze(0).transpose(1,2))
            # logits = model(data)
            loss1 = F.cross_entropy(logits, labels)
            logits = torch.max(F.softmax(logits, dim=1))
            curr_pred = [logits.cpu().detach().numpy()]
...
def train_paseCNN_class_models(train_loader, test_loader, k_round):
    ...
    pLSTM_model = paseLSTM_model(256, 100, 3, 2)

    if CUDA:
        pLSTM_model = pLSTM_model.cuda()

    optimizer = optim.Adam(filter(lambda p: p.requires_grad, pLSTM_model.parameters()), lr=0.0001, weight_decay=0.0001)
    losses = []
    for i in range(EPOCHS): #n_epochs
        pLSTM_model, loss = train_epoch(pLSTM_model, train_loader, optimizer, mode='pase')
        losses.append(sum(loss))
    torch.save(pLSTM_model.state_dict(), 'models/' + args.img_name + 'trained_pase_model_model'+str(k_round)+'.pth')

    print('Done paseCNN_model Training')

    pred, true, all_spks, spk_wise, _ = test_epoch(pLSTM_model, test_loader, None, None)
...
def test_paseCNN_class_models(test_loader, k_round):
    ...
    pLSTM_model.load_state_dict(torch.load(path))
...
if args.data == 'All':
    ...
    pos_covid_csv = '../chile_data/COVID-19_2020_CHILE/all_pos_speaker_list_u.csv' # 250 
    neg_covid_csv ='../chile_data/COVID-19_2020_CHILE/all_neg_speaker_list_u.csv' # 106
